...
func setPlayback(_ resolve: @escaping RCTPromiseResolveBlock,
  ...
  DispatchQueue.main.async {
    do {
      ...
      // clear any previous speaker override from call mode
      try session.overrideOutputAudioPort(.none)

      // fully deactivate first so iOS can drop call-style config
      try session.setActive(false, options: [])

      // switch to true media playback mode
      try session.setCategory(.playback, mode: .default, options: [])
      let options = session.categoryOptions

      print("[AUDIO_OPTIONS] raw:", options.rawValue)
      print("[AUDIO_OPTIONS] duckOthers:",
            options.contains(.duckOthers))
      print("[AUDIO_OPTIONS] mixWithOthers:",
            options.contains(.mixWithOthers))
      print("[AUDIO_OPTIONS] interruptSpokenAudioAndMixWithOthers:",
            options.contains(.interruptSpokenAudioAndMixWithOthers))
      print("[AUDIO_OPTIONS] category:", session.category.rawValue)
      print("[AUDIO_OPTIONS] mode:", session.mode.rawValue)

      // reactivate in playback mode
